...
const Sync = {
  ...
  async ListenerMarketPlaceSelltoken(fromBlockNumber, status) {
    if (status) {
  ...
  async ListenerMarketPlaceBuyToken(fromBlockNumber, status) {
    if (status) {
      const actualBlockNumber = (await provider.getBlockNumber()) - 1;
  ...
  async ListenerMarketPlaceRemoveToken(fromBlockNumber, status) {
    if (status) {
      const actualBlockNumber = (await provider.getBlockNumber()) - 1;
        ...
        for (let i = 0; i < events.length; i++) {
          ...
          try {
            let response = await axios.delete(
              `${endPointPostEvents}listings/${eventData.collection}/${eventData.tokenId}/${eventData.seller}`,
              {
                headers: {
                  Authorization: API_BEARER_TOKEN,
                  "Content-Type": "application/json",
  ...
  async ListenerSkillTransfer(fromBlockNumber, status) {
    if (status) {
      const actualBlockNumber = (await provider.getBlockNumber()) - 1;
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        if (
          events[i].args[0] !== "0x0000000000000000000000000000000000000000"
        ) {
  ...
  async ListenerSkinTransfer(fromBlockNumber, status) {
    if (status) {
      const actualBlockNumber = (await provider.getBlockNumber()) - 1;
  ...
  async ListenerERC1155MockUpTransferSingle(fromBlockNumber, status) {
    if (status) {
      const actualBlockNumber = (await provider.getBlockNumber()) - 1;
      ...
      for (let i = 0; i < highestBlockTransactions.length; i++) {
        ...
        console.log("ID:", highestBlockTransactions[i]);
  ...
  async ListenerERC1155MockUpTransferBatch(fromBlockNumber, status) {
    if (status) {
      const actualBlockNumber = (await provider.getBlockNumber()) - 1;
